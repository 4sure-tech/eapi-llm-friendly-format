...
The _**Retrieve Matching Credentials for A Proof Request API**_ enables the LOB to retrieve all stored credentials from the LOB's digital wallet that match the constraints of the Verifier's proof request. The LOB will then select one of the returned credentials in the credential presentation response sent back to the Verifier.

{% openapi-operation spec="enterpriseapi-holder" path="/api/lob/{lob_id}/{proof_id}/matching-credentials" method="get" %}
